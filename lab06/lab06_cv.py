...
sources_folds = getKFolds(sorted(glob("im/road?.png")), 4)
blocks_folds = getKFolds(sorted(glob("im/road?_blocks.png")), 4)
...
for sources, blocks in zip(sources_folds, blocks_folds):
    black_block_train = []
    white_block_train = []
    black_feature_count = 0
    white_feature_count = 0

    for source, block in zip(sources[0:3], blocks[0:3]):
    ...
    for source, block in zip(sources[3:], blocks[3:]):
        im = cv2.imread(source, 1)
        block_image = cv2.imread(block, 0)
        ...
        precision = precision_score(truth, output, average='weighted')
        total_precision += precision
        print("precision:", precision)

        recall = recall_score(truth, output, average='weighted')
        total_recall += recall
        print("recall:", recall, "\n")
        ...
        cv2.imwrite("rfc_cv_predicted_image_" + str(c) + ".png", final_image*255)
        cv2.waitKey()
        cv2.destroyWindow("image with prediction overlay")
        
        c += 1

avg_accuracy = total_accuracy/c
print("avg accuracy:", avg_accuracy)
avg_precision = total_precision/c
print("avg precision:", avg_precision)
avg_recall = total_recall/c
